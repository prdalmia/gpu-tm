...
__constant__ int nnodesd, nbodiesd, *errd, *sortd, *childd, *countd, *startd, *nextd;
...
typedef int2 version_ptr; 
__device__ version_ptr freetopd;

// TM stubs
bool useTM = false; 
// #define ABORT_HANDLER
__device__ __noinline__ void __tbegin() { }
#ifdef ABORT_HANDLER
__device__ __noinline__ int __tcommit(int dummyinput) { return 1; }
__device__ __noinline__ void __trollback() { }
#else
__device__ __noinline__ void __tcommit() { }
#endif
__device__ __noinline__ void __taccessmode(int readwritemode) { }
...
__global__ void InitializationKernel()
{
  ...
  if (i == 0) {
    *errd = 0;
    ...
    maxdepthd = 1;
    blkcntd = 0;
    nextd[nnodesd] = nnodesd - 1; 
    freetopd.x = nnodesd - 9; 
  }
  ...
  // this should be on BoundingBoxKernel, but kept here for simplicity 
  i = threadIdx.x + blockIdx.x * blockDim.x;
  for (; i < nnodesd; i += inc) {
    nextd[i] = i - 1;
  }
...
#pragma unroll 8
      ...
      bottomd = j;
      // preallocate first level child nodes
      #if 1
      for (i = 0; i < 8; i++) {
        int cell = (bottomd--) - 1; 
        float r_child = radiusd; 
        float px = (i & 1) * r_child;
        float py = ((i >> 1) & 1) * r_child;
        float pz = ((i >> 2) & 1) * r_child;
        r_child *= 0.5f; 
        massd[cell] = -1.0f; 
        startd[cell] = -1; 
        posxd[cell] = posxd[nnodesd] - r_child + px; 
        posyd[cell] = posyd[nnodesd] - r_child + py; 
        poszd[cell] = poszd[nnodesd] - r_child + pz; 
        #pragma unroll 8
        for (int c = 0; c < 8; c++) childd[cell*8+c] = -1;
        childd[nnodesd*8+i] = cell;
      }
      #endif
      stepd++;
    }
...
__device__ int alloc_node()
{
   ...
   do {
      ...
      // newT.si32[1]++;
      newT.x = nextd[n];
   } while (atomicCAS(&freetopd.x, T.x, newT.x) != T.x);
   __taccessmode(0x13);
   return n;
}

__device__ void free_node(int n)
{
   __taccessmode(0x02);
   version_ptr T, newT;
   do {
...
__global__ void TreeBuildingKernel_TM()
{
  register int i, j, k, depth, localmaxdepth, newbody, inc;
  register float x, y, z, r;
  register float px, py, pz;
  register int ch, n, cell, leafchild;
  ...
  while (i < nbodiesd) {
    ...
    __tbegin(); 
      ...
      ch = childd[n*8+j];
    __tcommit(); 
    // follow path to leaf cell before starting transaction
    while (ch >= nbodiesd) {
      n = ch;
      depth++;
      r *= 0.5f;
    ...
    __tbegin(); 
      ch = childd[n*8+j];
      // follow path to leaf cell (if it was aborted)
      while (ch >= nbodiesd) {
        n = ch;
        depth++;
        r *= 0.5f;
      ...
      if (ch == -1) {
        ...
        // create new cell(s) and insert the old body, leave the new body for next try
        depth++;
        ...
        childd[leafchild] = cell;

        // setting the next node for checking to the inserted cell 
        // (alternative is to reset everything, including radius, and start from the root)
        n = cell;
      }
    #ifdef ABORT_HANDLER
    if (__tcommit(0) == 0) { 
...
__global__ void TreeBuildingKernel_TM_atomcommit()
{
  register int i, j, k, depth, localmaxdepth, newbody, inc;
  register float x, y, z, r;
  register float px, py, pz;
...
int main(int argc, char *argv[])
{
  ...
  int *errl, *sortl, *childl, *countl, *startl, *nextl;
  float *massl;
  ...
  if (argc != 3 && argc != 4) {
    fprintf(stderr, "\n");
    fprintf(stderr, "arguments: number_of_bodies number_of_timesteps use_tm\n");
    exit(-1);
  } 
  if (argc == 4) {
    if (strcmp(argv[3], "-tm") == 0) {
      useTM = true; 
    }
  }
  ...
  for (run = 0; run < 1; run++) {
    ...
    if (run == 0) {
      ...
      if (cudaSuccess != cudaMalloc((void **)&nextl, sizeof(int) * (nnodes+1))) fprintf(stderr, "could not allocate nextd\n");  CudaTest("couldn't allocate nextd");
      ...
      if (cudaSuccess != cudaMemcpyToSymbol(nextd, &nextl, sizeof(int))) fprintf(stderr, "copying of nextl to device failed\n");  CudaTest("nextl copy to device failed");
    ...
    for (step = 0; step < timesteps; step++) {
      ...
      cudaEventRecord(start, 0);
      if (useTM) {
         TreeBuildingKernel_TM<<<blocks*FACTOR2, THREADS2>>>();
      } else {
