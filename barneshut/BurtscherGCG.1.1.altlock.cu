...
__global__ void TreeBuildingKernel()
{
  ...
  while (i < nbodiesd) {
    ...
    {  
      locked = n*8+j;
      int ch_cas = atomicCAS(&childd[locked], ch, -2); // try to obtain the lock 
      if (ch_cas != -2) {  // skip if child pointer is locked and try again later 
        ch = ch_cas; 
      // if (ch == atomicCAS(&childd[locked], ch, -2)) {  // try to lock
        if (ch == -1) {
          // if null, just insert the new body
          // childd[locked] = i;
          atomicExch(&childd[locked], i);
...
#pragma unroll 8
          ...
          __threadfence();
          // childd[locked] = patch;
          atomicExch(&childd[locked], patch);
        }
